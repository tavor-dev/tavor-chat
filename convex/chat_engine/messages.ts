...
export const searchMessages = action({
  args: {
    ...
    searchAllMessagesForUserId: v.optional(v.id("users")),
  ...
  handler: async (ctx, args): Promise<MessageDoc[]> => {
    ...
    if (args.text) {
      textSearchMessages = await ctx.runQuery(
        api.chat_engine.messages.textSearch,
        {
    ...
    if (args.vector) {
      ...
      const messages: MessageDoc[] = await ctx.runQuery(
        internal.chat_engine.messages._fetchSearchMessages,
        {
          searchAllMessagesForUserId: args.searchAllMessagesForUserId,
          threadId: args.threadId,
          vectorIds,
