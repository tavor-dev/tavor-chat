...
export { vMessageDoc, vThreadDoc } from "@cvx/schema";
...
export class Agent<AgentTools extends ToolSet> {
  ...
  async createThread<ThreadTools extends ToolSet | undefined = undefined>(
    ...
    if (!("runAction" in ctx)) {
      return { threadId: threadDoc._id as Id<"threads"> };
    }
    const { thread } = await this.continueThread(ctx, {
      threadId: threadDoc._id as Id<"threads">,
      userId: args?.userId,
  ...
  async listMessages(
    ...
    return ctx.runQuery(api.chat_engine.messages.listMessagesByThreadId, {
      order: "desc",
      ...args,
    });
  }
  ...
  async syncStreams(
    ...
    if (args.streamArgs.kind === "list") {
      return {
        kind: "list",
        messages: await ctx.runQuery(api.chat_engine.streams.list, {
          threadId: args.threadId,
        }),
      };
    } else {
      return {
        kind: "deltas",
        deltas: await ctx.runQuery(api.chat_engine.streams.listDeltas, {
          threadId: args.threadId,
  ...
  async generateAndSaveEmbeddings(
    ...
    const messages = (
      await ctx.runQuery(api.chat_engine.messages.getMessagesByIds, {
    ...
    await ctx.runMutation(api.chat_engine.vector.index.insertBatch, {
  ...
  async saveStep<TOOLS extends ToolSet>(
    ...
    const saved = await ctx.runMutation(api.chat_engine.messages.addMessages, {
      userId: args.userId,
      threadId: args.threadId,
      agentName: this.options.name,
      promptMessageId: args.promptMessageId,
  ...
  async saveObject(
    ...
    await ctx.runMutation(api.chat_engine.messages.addMessages, {
      userId: args.userId,
      threadId: args.threadId,
      promptMessageId: args.promptMessageId,
      failPendingSteps: false,
  ...
  async _searchOptionsWithEmbeddingAndDefaults(
    ctx: RunActionCtx,
    { userId, threadId }: { userId?: Id<"users">; threadId?: Id<"threads"> },
    contextOptions: ContextOptions,
    messages: CoreMessage[],
