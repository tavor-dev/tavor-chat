...
import type { ActionCtx, RunMutationCtx } from "./client/types";
...
): Promise<{ message: SerializedMessage; fileIds?: Id<"files">[] }> {
  const { id: _, experimental_providerMetadata, ...message } = messageWithId;
  const { content, fileIds } = await serializeContent(ctx, message.content);
  return {
    message: {
...
export async function serializeNewMessagesInStep<TOOLS extends ToolSet>(
  ...
  const messages: MessageWithMetadata[] = await Promise.all(
    (step.toolResults.length > 0
      ...
      const { message, fileIds } = await serializeMessage(ctx, messageWithId);
      return {
        message,
...
export async function serializeContent(
  ctx: ActionCtx | RunMutationCtx,
  content: Content,
): Promise<{ content: SerializedContent; fileIds?: Id<"files">[] }> {
