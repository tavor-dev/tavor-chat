...
import type {
  ...
  StreamDelta,
  TextStreamPart,
} from "@cvx/chat_engine/validators";
...
describe("mergeDeltas", () => {
  it("merges a single text-delta into a message", () => {
    const streamId = "s1" as Id<"streamingMessages">;
  ...
  it("handles tool-call and tool-result parts", () => {
    const streamId = "s2" as Id<"streamingMessages">;
  ...
  it("returns changed=false if no new deltas", () => {
    const streamId = "s3" as Id<"streamingMessages">;
  ...
  it("handles multiple streams and sorts by order/stepOrder", () => {
    const s1 = makeStreamMessage("s1" as Id<"streamingMessages">, 1, 0);
    const s2 = makeStreamMessage("s2" as Id<"streamingMessages">, 2, 0);
    const deltas = [
      makeDelta("s2" as Id<"streamingMessages">, 0, 3, [
        { type: "text-delta", textDelta: "B" },
      ]),
      makeDelta("s1" as Id<"streamingMessages">, 0, 3, [
  ...
  it("does not duplicate text content when merging sequential text-deltas", () => {
    const streamId = "s4" as Id<"streamingMessages">;
  ...
  it("does not duplicate reasoning parts", () => {
    const streamId = "s6" as Id<"streamingMessages">;
  ...
  it("applyDeltasToStreamMessage is idempotent and does not duplicate content", () => {
    const streamId = "s7" as Id<"streamingMessages">;
  ...
  it("mergeDeltas is pure and does not mutate inputs", () => {
    const streamId = "s8" as Id<"streamingMessages">;
